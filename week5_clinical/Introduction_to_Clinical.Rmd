---
title: "COMPLETED Intro to TCGA Clinical Data"
date: 07/17/22
updated: 09/26/24
---
...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/home1/odani/490_cluster/analysis_data")) 
```
...
```{r}
write.csv(clinic, "/home1/odani/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/odani/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 1. 1097 rows, 112 columns
# 2. gender column is chr type
# 3. 5th patient is 18371 days old (~50)
# 4. Age at last follow up = Age - days since last follow up = 20211 - 3767 = 1644 days or ~45
```
...
plot(clinic$age_at_diagnosis, clinic$lymph_nodes_examined_count,
     main = "Diagnosis Age vs Lymph Nodes", xlab = "Age at Diagnosis", 
     ylab = "Number of Lymph Nodes", col = "red")
```
...
boxplot(as.numeric(clinic$lymph_nodes_examined_count) ~ clinic$tumor_status, date = clinic,
        xlab = "Tumor Status", ylab = "Number of Lymph Nodes")
```
...
```{r}
no_middle_clinic <- data.frame(under30_over60_mask)
```
...
```{r}
cleaned_clinic$age_category <- ifelse(clinic$age_at_diagnosis <= 35, "Young", 
...
# 1. What does the y axis represent?
  # y axis represents survival rate of patients in age range at that time

# 2. What does the x axis represent?
 # x axis represents time after diagnosis
...
# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? 
 ...
 # p value is 0.2, which is not considered statistically significant, implies that there is no causation between age and breast cancer survival
